...
try {
  ...
  // MEN-0001 = the oldest case, while every Mentor case came from this import.
  const handMade = (await q('select count(*)::int as n from public.mentor_cases where import_ref is null')).rows[0].n;
  if (handMade === 0) {
    await q('alter table public.mentor_cases alter column case_no set generated by default');
    await q(`update public.mentor_cases m set case_no = n.rn from (
               select id, row_number() over (order by case_date nulls last, import_ref) as rn from public.mentor_cases
             ) n where n.id = m.id`);
    await q('alter table public.mentor_cases alter column case_no set generated always');
    await q(`select setval(pg_get_serial_sequence('public.mentor_cases', 'case_no'), (select coalesce(max(case_no), 1) from public.mentor_cases))`);
  }
  ...
  const summary = `${records.length} cases${removed.rowCount ? ` (replacing ${removed.rowCount} from an earlier import)` : ''}; ${samples.rowCount} sample cases removed; numbered MEN-0001 (oldest) onwards.`;
  if (WRITE) {
    await q('commit');
